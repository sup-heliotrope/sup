...
class MaildirRoot < Source
  ...
  def initialize uri, usual=true, archived=false, id=nil, labels=[]
    ...
    @archive = MaildirSub.new @root, @archive_folder, :archive
    @inbox   = MaildirSub.new @root, @inbox_folder, :inbox
    @sent    = MaildirSub.new @root, @sent_folder, :sent
    @drafts  = MaildirSub.new @root, @drafts_folder, :drafts
    @trash   = MaildirSub.new @root, @trash_folder, :trash

    # scan for other non-special folders
    debug "setting up non-special folders.."
    @maildirs = Dir.new(@root).entries.select { |e| File.directory? File.join(@root,e) and e != '.' and e != '..' and !@special_folders.member? e }.each { |d| MaildirSub.new(@root, d, :generic) }.to_a
    debug "maildir subs setup done."

  end

  # A class representing one sub-directory (label) in the maildir root
  class MaildirSub
    def initialize root, dir, type=:generic
      @root = root
